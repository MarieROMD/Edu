    ...
    <table class="crud-table">
      ...
      <tbody id="eventBody">
        ...
        <tr id="erow<?= $e['id'] ?>">
          ...
          <td style="color:#9ca3af;font-size:.8rem"><?= date('d M Y', strtotime($e['date_event'])) ?></td>
          <td style="font-size:.8rem">
            <?= $e['total_participants'] ?>/<?= $e['capacite'] ?>
            <?php if ($e['places_restantes'] <= 0): ?>
              <span style="color:#f87171;font-size:.75rem"> · Complet</span>
            <?php endif; ?>
          </td>
          <td>
            <button class="btn-edit"
              onclick="EventCRUD.openModal(
                <?= $e['id'] ?>,
                '<?= addslashes($e['titre']) ?>',
            ...
            <button class="btn-delete" onclick="EventCRUD.del(<?= $e['id'] ?>, '<?= addslashes($e['titre']) ?>')">
              🗑️
            </button>
          </td>
        </tr>
...
const EventCRUD = (() => {
  ...
  async function save() {
    const data = await post('/admin/events/save', {
      titre:       document.getElementById('e_titre').value.trim(),
      description: document.getElementById('e_description').value.trim(),
      date_event:  document.getElementById('e_date').value,
      capacite:    document.getElementById('e_capacite').value,
    ...
    tr.innerHTML = `
      ...
      <td style="font-size:.8rem">0/${e.capacite}</td>
      <td>
        <button class="btn-edit"
          onclick="EventCRUD.openModal(${e.id},'${e.titre}','${e.description}','${e.date_event}',${e.capacite},'${e.image_url}')">
          ✏️
        </button>
        <button class="btn-delete" onclick="EventCRUD.del(${e.id},'${e.titre}')">🗑️</button>
      </td>`;
    document.getElementById('eventBody').prepend(tr);

    document.getElementById('e_titre').value       = '';
    document.getElementById('e_description').value = '';
    document.getElementById('e_date').value        = '';
    document.getElementById('e_capacite').value    = '';
  ...
  async function update() {
    ...
    const data = await post('/admin/events/save', {
      ...
      date_event:  document.getElementById('m_date').value,
      capacite:    document.getElementById('m_capacite').value,
    ...
    if (row) {
      row.cells[0].innerHTML = '<strong>' + document.getElementById('m_titre').value + '</strong>';
      row.cells[1].textContent = formatDate(document.getElementById('m_date').value);
      row.cells[2].textContent = row.cells[2].textContent.split('/')[0] + '/' + document.getElementById('m_capacite').value;
    }
  }
  ...
  async function del(id, titre) {
    ...
    const data = await post('/admin/events/delete', { id });
